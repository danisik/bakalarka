...
\input{chapters/pdf_format.tex}
\input{chapters/php_knihovny.tex}
\input{chapters/navrh_modulu.tex}
\input{chapters/implementace_modulu.tex}

% implementace modulu (vyber vhodnych knihoven (popis vyhod nevyhod, zaver vyberu), popis souborů, popis důležitějších funkcí (redukce délky a fontu jména), popis chyby knihoven (mpdf textarea text chyba atp.), výsledný vzhled, omezení (verze php, rychlost atp.) A MOZNA NECO DALSIHO)

%\ porovnani vysledku implementovaneho modulu se starym modulem (vzhled, funkcionalita, kod (i komentare))

% rozšiřitelnost ?? (přidání nového elementu, přidání více prvků, ...)

\input{chapters/testovani.tex} % automatické testy ?? + vstupní data + výsledky, webové prohlížeče, pdf prohlížeče, popis chyb ??
